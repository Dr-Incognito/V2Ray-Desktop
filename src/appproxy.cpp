...
QStringList AppProxy::getAppConfigErrors(const QJsonObject& appConfig) {
  QStringList errors;
  errors.append(getStringConfigError(appConfig, "language", "Language"));
  errors.append(
    getStringConfigError(appConfig, "serverProtocol", "Local Server Protocol"));
  errors.append(getStringConfigError(
    appConfig, "serverIp", "Listening IP Address",
    {
      std::bind(&AppProxy::isIpAddrValid, this, std::placeholders::_1),
    }));
  getNumericConfigError(appConfig, "serverPort", "Listening Port", 1, 65535);
  getNumericConfigError(appConfig, "pacPort", "PAC Server Port", 1, 65535);
  ...
  // Remove empty error messages generated by getNumericConfigError()
  errors.removeAll("");
  return errors;
}
...
void AppProxy::setGfwListUrl(QString gfwListUrl) {
  ...
  QString error         = getStringConfigError(
    appConfig, "gfwListUrl", "GFW List URL",
...
QStringList AppProxy::getV2RayServerConfigErrors(
  ...
  QStringList errors;
  errors.append(
    getStringConfigError(serverConfig, "serverName", "Server Name"));
  errors.append(getStringConfigError(
    serverConfig, "serverAddr", "Server Address",
    {
      std::bind(&AppProxy::isIpAddrValid, this, std::placeholders::_1),
      std::bind(&AppProxy::isDomainNameValid, this, std::placeholders::_1),
    }));
  errors.append(
    getNumericConfigError(serverConfig, "serverPort", "Server Port", 0, 65535));
  errors.append(getStringConfigError(serverConfig, "id", "ID"));
  errors.append(
    getNumericConfigError(serverConfig, "alterId", "Alter ID", 0, 65535));
  errors.append(getStringConfigError(serverConfig, "security", "Security"));
  errors.append(getNumericConfigError(serverConfig, "mux", "MUX", -1, 1024));
  errors.append(getStringConfigError(serverConfig, "network", "Network"));
  errors.append(
    getStringConfigError(serverConfig, "networkSecurity", "Network Security"));
...
QStringList AppProxy::getV2RayStreamSettingsErrors(
  ...
  if (network == "kcp") {
    errors.append(
      getNumericConfigError(serverConfig, "kcpMtu", "MTU", 576, 1460));
    errors.append(
      getNumericConfigError(serverConfig, "kcpTti", "TTI", 10, 100));
    errors.append(getNumericConfigError(serverConfig, "kcpUpLink",
                                        "Uplink Capacity", 0, -127));
    errors.append(getNumericConfigError(serverConfig, "kcpDownLink",
                                        "Downlink Capacity", 0, -127));
    errors.append(getNumericConfigError(serverConfig, "kcpReadBuffer",
                                        "Read Buffer Size", 0, -127));
    errors.append(getNumericConfigError(serverConfig, "kcpWriteBuffer",
                                        "Write Buffer Size", 0, -127));
    errors.append(
      getStringConfigError(serverConfig, "packetHeader", "Packet Header"));
  } else if (network == "ws" || network == "http") {
    errors.append(getStringConfigError(
      serverConfig, "networkHost", "Host",
      {std::bind(&AppProxy::isDomainNameValid, this, std::placeholders::_1)}));
    errors.append(getStringConfigError(serverConfig, "networkPath", "Path"));
  } else if (network == "domainsocket") {
    errors.append(getStringConfigError(
      serverConfig, "domainSocketFilePath", "Socket File Path",
      {std::bind(&AppProxy::isFileExists, this, std::placeholders::_1)}));
  } else if (network == "quic") {
    errors.append(
      getStringConfigError(serverConfig, "quicSecurity", "QUIC Security"));
    errors.append(
      getStringConfigError(serverConfig, "packetHeader", "Packet Header"));
    errors.append(getStringConfigError(serverConfig, "quicKey", "QUIC Key"));
  }
  return errors;
}
...
QString AppProxy::getStringConfigError(
  ...
  if (!serverConfig.contains(key) || serverConfig[key].toString().isEmpty()) {
    return QString(tr("Missing the value for '%1'.")).arg(name);
  }
  if (checkpoints.size() > 0) {
    ...
    if (!isMatched) {
      return QString(tr("The value for '%1' seems invalid.")).arg(name);
    }
  }
  return "";
}
...
QString AppProxy::getNumericConfigError(const QJsonObject& serverConfig,
  ...
  if (!serverConfig.contains(key) || serverConfig[key].toString().isEmpty()) {
    ...
    if (!isConverted) {
      return QString(tr("'%1' seems invalid.")).arg(name);
    } else if (upperBound == -127 && value < lowerBound) {
      return QString(tr("'%1' should above %2."))
        .arg(name, QString::number(lowerBound));
    } else if (value < lowerBound || value > upperBound) {
      return QString(tr("'%1' should between %2 and %3."))
...
QStringList AppProxy::getShadowsocksServerConfigErrors(
  ...
  errors.append(
    getNumericConfigError(serverConfig, "serverPort", "Server Port", 0, 65535));
  errors.append(getStringConfigError(serverConfig, "encryption", "Security"));
  errors.append(getStringConfigError(serverConfig, "password", "Password"));
...
void AppProxy::addSubscriptionUrl(QString subsriptionUrl) {
  if (subsriptionUrl.isEmpty()) {
    emit serverConfigError(tr("Missing the value for 'Subscription URL'."));
    return;
  } else if (!isUrlValid(subsriptionUrl)) {
    emit serverConfigError(tr("'Subscription URL' seems invalid."));
    return;
  }
